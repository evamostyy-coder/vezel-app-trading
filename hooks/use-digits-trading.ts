...
interface UseDigitsTradingReturn {
  ...
  /** Raw price history of the active symbol — shared with the bot engine. */
  prices: number[];
  /** Last-digit of every price in the buffer — the single digit source shared with the bot. */
  digits: number[];
...
export function useDigitsTrading({ ws, isConnected, isExhausted, isAuthenticated, onAuthWSFailed }: UseDigitsTradingParams): UseDigitsTradingReturn {
  ...
  // Single source of truth for digits: derive once from the shared price
  // buffer, then reuse for both the UI stats bar and the bot engine so the
  // two decision engines can never compute divergent digit values.
  const digits = useMemo(
    () => prices.map((p) => getLastDigit(p, pipSize)),
    [prices, pipSize]
  );
  ...
  const digitStats: DigitStats = useMemo(() => {
    ...
    return {
      counts,
      percentages: counts.map((c) => (total > 0 ? (c / total) * 100 : 0)),
      totalTicks: total,
    };
  }, [digits]);
